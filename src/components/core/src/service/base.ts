...
import {PureEventManager} from "../event";
import {PureValidateException} from "../exception/validate";
...
export abstract class BaseService {
  // 分页配置
  @Config("pure")
  private _coolConfig: PureConfig;
  ...
  @Inject()
  coolEventManager: PureEventManager;
  ...
  async sqlRenderPage(sql, query, autoSort = true, connectionName?) {
    ...
    if (order && sort && autoSort) {
      if (!(await this.paramSafetyCheck(order + sort))) {
        throw new PureValidateException("非法传参~");
      }
  ...
  private checkSort(sort) {
    if (!["desc", "asc"].includes(sort.toLowerCase())) {
      throw new PureValidateException("sort 非法传参~");
    }
    return sort;
  }
  ...
  async info(id: any, infoIgnoreProperty?: string[]) {
    if (!this.entity) throw new PureValidateException(ERRINFO.NOENTITY);
    if (!id) {
      throw new PureValidateException(ERRINFO.NOID);
    }
  ...
  private async getOptionFind(query, option: QueryOp) {
    ...
    if (sort && order) {
      ...
      if (sorts.length != orders.length) {
        throw new PureValidateException(ERRINFO.SORTFIELD);
      }
