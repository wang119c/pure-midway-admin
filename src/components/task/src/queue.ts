...
import { BasePureQueue } from './base';
import { PURE_TASK_KEY } from './decorator/queue';
...
export class PureQueueHandle {
   ...
   */
  async scan() {
    const modules = listModule(PURE_TASK_KEY);
    for (let mod of modules) {
      const cls: BasePureQueue = await this.app
   ...
   * @param mod
   */
  async createQueue(cls: BasePureQueue, mod: any) {
    ...
    const name = mod.name;
    const config = getClassMetadata(PURE_TASK_KEY, mod);
    const opts = {
