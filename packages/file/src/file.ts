...
import { Mode, PureFileConfig, MODETYPE, CLOUDTYPE } from './interface';
import {PureCommException} from "./exception/comm";
...
export class PureFile {
  ...
  @Init()
  async init(config: PureFileConfig) {
  ...
  private async local(ctx) {
    try {
      ...
      if (_.isEmpty(ctx.files)) {
        throw new PureCommException('上传文件为空');
      }
      ...
      throw new PureCommException('上传失败');
    }
  }
}
